...
- `P1` OS → 앱 드래그앤드롭 (Finder/탐색기에서 파일을 끌어다 패널에 드롭 → 복사, 사용자 요청으로 추가) — ✅ 완료. `desktop_drop` 사용, 목적지가 FTP 패널이면 업로드로 자연스럽게 이어짐. **앱 → OS로 내보내는 반대 방향은 취소** (Flutter 기본 `Draggable`은 앱 내부 드래그만 지원, 네이티브 드래그 세션을 만들려면 별도 패키지/플랫폼 코드 필요 — 난이도 대비 실익이 낮다고 판단해 사용자가 진행 취소 결정. 보류가 아니라 비목표로 확정)
...
- 앱 → OS 드래그앤드롭 (앱에서 파일을 끌어 Finder/탐색기에 드롭 → 이동/복사). OS → 앱 방향은 완료했지만 반대 방향은 네이티브 드래그 세션이 필요해 난이도가 훨씬 높음 — 사용자가 진행 취소 결정
...
15. 런타임 글자 크기 조절 — **완료** (사용자 요청):
    ...
    - 매칭되는 새 아이콘이 없어 Material 아이콘 사용(`format_size`/
      `text_increase`/`text_decrease`)
