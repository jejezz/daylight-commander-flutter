...
class FileTypeStyle {
  ...
  static const Map<String, String> _namedFolderAssets = {
    'documents': '$_folderBase/icons8-documents-folder.svg',
    '문서': '$_folderBase/icons8-documents-folder.svg',
    'games': '$_folderBase/icons8-games-folder.svg',
    '게임': '$_folderBase/icons8-games-folder.svg',
  };

  /// 폴더 이름으로 테마 폴더 아이콘을 고른다 (예: "Documents" → 문서 폴더 아이콘).
  static String folderAsset(String folderName) =>
      _namedFolderAssets[folderName.trim().toLowerCase()] ?? genericFolderAsset;
  ...
  static FileCategory categoryForExtension(String extension) {
    ...
    if (const {'dart', 'java', 'py', 'js', 'ts', 'json', 'xml', 'html', 'htm',
        'css', 'sql', 'yaml', 'yml'}.contains(ext)) {
  ...
  static Color colorFor(FileCategory category, {required bool isDark}) {
    switch (category) {
      ...
      case FileCategory.networkDrive:
